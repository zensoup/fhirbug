...
class FHIRAbstractBase(object):
    ...
    def _with_json_dict(cls, jsondict):
        ...
        """
        ## We accept input of type dict or cls
        if not isinstance(jsondict, dict) and not isinstance(jsondict, cls):
            raise TypeError("Can only use `_with_json_dict()` on {} with a dictionary or {}, got {}"
                .format(type(self), cls.__name__, type(jsondict)))
        return cls(jsondict)
    ...
    def update_with_json(self, jsondict):
        ...
        ## Convert to json if jsondict is a Resource
        if hasattr(jsondict, 'as_json'):
            jsondict = jsondict.as_json()
